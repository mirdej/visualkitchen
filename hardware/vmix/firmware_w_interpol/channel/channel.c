...
const 	unsigned char  	barleds[]  PROGMEM =  {0x00,0x40,0x60,0x70,0x78,0x7c,0x7e,0x7f,0x7f,0x7f};
const 	unsigned char  	pointleds[]  PROGMEM =  {0x00,0x40,0x20,0x10,0x08,0x04,0x02,0x01,0x01,0x01};
const	unsigned char 	ledtimes[] PROGMEM = {90,75,62,50,40,30,22,15,10,5,2,0,0,1,7,16,29,45,66,90,80,72,63,56,48,42,35,30,24,20,15,12,8,6,3,2,0,0,0,0,0,0,1,2,3,4,5,7,9,11,13,15,18,21,23,27,30,33,37,41,45,49,53,58,63,68,73,78,84,90,81,73,66,58,52,45,40,34,29,24,20,16,13,10,7,5,3,1,0,0,0,0,0,0,1,2,3,5,6,8,10,12,15,18,20,24,27,30,34,38,42,47,51,56,61,66,72,78,83,90,68,50,35,22,12,5,1,0,0,0,1,2,3,5,7,10,12,15,18,22,26,30,35,40,45,50,56,62,68,75,82,90,82,75,68,62,56,50,45,40,35,30,26,22,18,15,12,10,7,5,3,2,1,0,0,0,0,0,2,3,6,8,12,15,20,24,30,35,42,48,56,63,72,80,90,83,76,70,64,58,53,48,43,38,34,29,26,22,19,16,13,10,8,6,4,3,2,1,0,0,0,0,0,1,3,5,7,10,13,16,20,24,29,34,40,45,52,58,66,73,81,90,76,64,53,43,34,26,19,13,8,4,2,0,90,90};


//----- Globals ---------------------------------------------------------

static unsigned char 		led_mux;
static unsigned char		led_patterns[8];
static unsigned char		led_alt_patterns[8];
static unsigned char		led_alt_times[8];
static unsigned char		led_phase,ledring_updating;
static unsigned char 		ring_display_mode[8];			// display modes for each ledring
static unsigned char 		stick;							// bitmask: which pots stick to their values?
static unsigned char		values[9];						// current values as output by object - not necessarily values of pots...

static unsigned char		interpol_presets[8][9];			

static unsigned char 		lastButtonState[3];
static unsigned int			buttonCounter[3];


static unsigned char		ad_mux;			// current ad input channel
static unsigned int			ad_values[8];	// sampled ad input values
static unsigned char		ad_smoothing;	// smoothing level of ad samples (0 -  15)
static unsigned char		ad_samplepause;	// counts up to ADC_PAUSE between samples

static unsigned char		interpol_angle, interpol_radius;


static unsigned char		txIndex;
static unsigned char 		rxBuffer[PACKETSIZE];
static unsigned char		txBuffer[PACKETSIZE];
...
void init(void) {
	...
	// preset 0 is initial setting
	for (j=0; j<9; j++) {
		values[j] = eepromRead(j);
	}
	// presets 1 - 8 can be played with the tactball
	for (i=0; i<8; i++) {
		for (j=0; j<9; j++) {
			interpol_presets[i][j] = eepromRead(i*9+j+9);
		}
	}
	// Initialize Timer 
...
void preset_store(unsigned char addr) {
	...
	// presets 1 - 8 can be interpolated in 2d with the tactball
	addr -= 1;
	if (addr < 8) {
		for (i = 0; i < 9; i++) {
			interpol_presets[addr][i] = values[i];
		}		
	}
}
...
unsigned char interpolate( unsigned char channel, unsigned char val0) {

	if (interpol_radius == 0) return val0;
	
	s16 temp;
	unsigned char alpha,l,u,val1,val2;
	
	l = interpol_angle / 32;
	u = l + 1;
	if (u > 7) u = 0;
	alpha = interpol_angle % 32;
	
	val1 = interpol_presets[l][channel];
	val2 = interpol_presets[u][channel];	
	
	if (alpha == 31) temp = val2;
	else if (alpha == 0) temp = val1;
	else {
		temp = 32 * val1 + alpha * ( val2 - val1 );
		temp = temp / 32;
	}
	
	if (interpol_radius > 32) return (unsigned char) temp;
	
	temp = 32 * val0 + interpol_radius * (temp - val0);
	temp = temp / 32;
...
unsigned char center_pad(unsigned int val) {
...
void checkSPI (void) {
	...
	if (PINB & (1 << PB4)) {   	// Slave is not selected - no ongoing transmission		
			if (txIndex) {
				...
				switch (rxBuffer[0]) {
					case REQ_GET_VALUES:
						interpol_angle 		= rxBuffer[2];
						interpol_radius 	= rxBuffer[3];
						break;
