...
static u08			interpol_angle, interpol_radius;
	...
	case SPI_STATE_DONE:
		...
		if (spi_current_slave >= 5) {
			...
			if (spi_queued_command) {
				...
				txBuffer[2] = interpol_angle;
				txBuffer[3] = interpol_radius;
			}
		}
...
uchar   usbFunctionSetup(uchar data[8]){
    ...
    if(data[1] == cmd_GET){       /* A BANG on Max object ->GET ALL VALUES*/
    	interpol_angle = data[2];
    	interpol_radius = data[3];
		usbMsgPtr = values;
